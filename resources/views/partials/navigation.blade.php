<nav class="navigation" role="navigation">
	<ul id="lbmenu" class="navigation__list">
		<li menuanchor="home" class="navigation__link active">
			<a class="navigation__item" href="#home" onclick="ga('send', 'event', 'navigation', 'jump to' , 'home')">Home</a>
		</li>
		<li menuanchor="lb_collection" class="navigation__link">
			<a class="navigation__item" href="#lb_collection" onclick="ga('send', 'event', 'navigation', 'jump to' , 'collection')">Collection</a>
		</li>
		<li menuanchor="brands" class="navigation__link">
			<a class="navigation__item" href="#brands" onclick="ga('send', 'event', 'navigation', 'jump to' , 'brand')">Brands</a>
		</li>
		<li menuanchor="projects" class="navigation__link">
			<a class="navigation__item" href="#projects" onclick="ga('send', 'event', 'navigation', 'jump to' , 'Projects')">Projects</a>
		</li>
		<li menuanchor="about" class="navigation__link">
			<a class="navigation__item" href="#about" onclick="ga('send', 'event', 'navigation', 'jump to' , 'About')">About</a>
		</li>
		<li menuanchor="contact" class="navigation__link">
			<a class="navigation__item on-modal navigation--highlight" data-modal="contact-modal" href="#contact" onclick="ga('send', 'event', 'navigation', 'jump to' , 'Contact')">Contact&nbsp;Us</a>
		</li>
	</ul>
</nav>
<div class="navigation__mobile_toggle">Menu</div>